Public Class ValiderAttributionMarche
    ...
    Private Sub ChargerListe()
        ...
        For Each rw As DataRow In dt.Rows
            NomListe.Add(rw(0).ToString & "_" & MettreApost(rw(1).ToString))
        Next
    End Sub
    Private Sub VerifierListe()
        query = "select S.CodeMem,S.NomMem, F.Validation from T_Commission as S , t_lotvalidationrapportcojo as F where F.Id_COJO=S.CodeMem and S.NumeroDAO='" & JugementOffres.CmbNumDoss.Text & "'"
        Dim dt As DataTable = ExcecuteSelectQuery(query)
