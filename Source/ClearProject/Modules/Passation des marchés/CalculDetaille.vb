...
Public Class CalculDetaille
    ...
    Private Sub calculprix()
        'Dim prixcorrige As String = ""
        'Dim soustract As String
        '
        'sql = "select sum(s.montantitem * d.qteitem), sf.refsoumis, sf.MontantPropose from t_soumisprixitemdqe s, t_dqeitem d, t_soumissionfournisseur sf, t_fournisseur f, t_lotdao l where d.refitem=s.refitem and s.refsoumis=sf.refsoumis and sf.codefournis=f.codefournis and sf.codelot=l.codelot and f.nomfournis='" & TxtNomSoumis.Text & "' and l.codelot='" & JugementOffres.CmbNumLot.Text & "' group by sf.refsoumis, sf.MontantPropose"
        'dr1 = ExecuteQuerry(query)
        'While dr1.Read

        '    prixcorrige = dr1(0).ToString
        '    soustract = IIf(dr1(0).ToString - dr1(2).ToString < "0", dr1(2).ToString - dr1(0).ToString, dr1(0).ToString - dr1(2).ToString)
        '    
        '   query= "update t_soumissionfournisseur set PrixCorrigeOffre='" & prixcorrige.ToString & "', ErreurCalcul='" & soustract.ToString & "' where RefSoumis='" & dr1(1).ToString & "'"
    ...
    Private Sub ChargerLesInfos()
        If (ReponseDialog <> "") Then
            ...
            For Each rw As DataRow In dt.Rows
                ...
                TxtMontLu.Text = IIf(IsNumeric(rw(1)) <> "0", AfficherMonnaie(rw(1).ToString), "0").ToString
                LblSigneErreur.Text = rw(12).ToString
                'TxtErrCalcul.Text = IIf(rw(3).ToString <> "0", AfficherMonnaie(rw(3).ToString), AfficherMonnaie(CalculerErreur(TxtMontLu.Text.Replace(" ", "")).ToString)).ToString
                TxtErrCalcul.Text = IIf(Not IsDBNull(rw(3)), AfficherMonnaie(rw(3).ToString), AfficherMonnaie(CalculerErreur(TxtMontLu.Text.Replace(" ", "")).ToString)).ToString
                TxtErrCalcul.Text = IIf(TxtErrCalcul.Text = "", 0, TxtErrCalcul.Text)
                TxtSomProvis.Text = IIf(rw(4).ToString = "", 0, AfficherMonnaie(rw(4).ToString))
                TxtMontRabais.Text = IIf(rw(6).ToString <> "", AfficherMonnaie(rw(6).ToString), "0")
                TxtDeviseOffre.Text = rw(0).ToString
                TxtTaux.Text = AfficherMonnaie(Taux(rw(0).ToString))
                TxtAjouts.Text = AfficherMonnaie(rw(7).ToString)
                TxtAjouts.Text = IIf(TxtAjouts.Text = "", 0, TxtAjouts.Text)
                TxtAjustements.Text = AfficherMonnaie(rw(8).ToString)
                TxtAjustements.Text = IIf(TxtAjustements.Text = "", 0, TxtAjustements.Text)
                TxtVariations.Text = AfficherMonnaie(rw(9).ToString)
                TxtVariations.Text = IIf(TxtVariations.Text = "", 0, TxtVariations.Text)

                TxtRang.Text = IIf(rw(11).ToString <> "0", rw(11).ToString & IIf(rw(11).ToString = "1", "er", "ème").ToString, "")
            Next
        End If
    End Sub
            ...
            If (rw(0).ToString <> "") Then
                If (IsNumeric(rw(0).ToString.Replace(" ", "")) = True) Then
                    If (JugementOffres.TxtTypeMarche.Text = "Fournitures" And IsNumeric(rw(1).ToString.Replace(" ", "")) = True) Then
                        montReel = montReel + (CDec(rw(0).ToString.Replace(" ", "")) * CDec(rw(1).ToString.Replace(" ", "")))
                    ElseIf (JugementOffres.TxtTypeMarche.Text = "Fournitures") Then
                        montReel = montReel + (CDec(rw(0).ToString.Replace(" ", "")))
                    End If
    ...
    Private Sub BtEnrgCalcul_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtEnrgCalcul.Click
        If (TxtPrixTotal.Text.Replace(" ", "") <> "") Then
            ...
            If nbsouslot > 0 Then
                query = "SELECT Attribution FROM t_dao WHERE NumeroDAO='" & JugementOffres.CmbNumDoss.Text & "' AND CodeProjet='" & ProjetEnCours & "'"
                Verif = ExecuteScallar(query)
                If Verif = "Lot" Then
                    query = "update t_soumissionfournisseur set VariationMineure='" & IIf(TxtVariations.Text.Replace(" ", "") <> "", TxtVariations.Text.Replace(" ", ""), "0") & "', Ajustements='" & IIf(TxtAjustements.Text.Replace(" ", "") <> "", TxtAjustements.Text.Replace(" ", ""), "0").ToString & "', AjoutOmission='" & IIf(TxtAjouts.Text.Replace(" ", "") <> "", TxtAjouts.Text.Replace(" ", ""), "0").ToString & "', MontantRabais='" & IIf(TxtMontRabais.Text.Replace(" ", "") <> "", TxtMontRabais.Text.Replace(" ", ""), "0").ToString & "', PrctRabais='" & IIf(TxtPrctRabais.Text.Replace(" ", "").Replace(",", ".") <> "", TxtPrctRabais.Text.Replace(" ", "").Replace(",", "."), "0").ToString & "', SomProvision='" & IIf(TxtSomProvis.Text.Replace(" ", "") <> "", TxtSomProvis.Text.Replace(" ", ""), "0").ToString & "', SigneErreur='" & LblSigneErreur.Text & "', MontantAvecMonnaie='" & IIf(TxtPrixEnDevise.Text.Replace(" ", "") <> "", TxtPrixEnDevise.Text.Replace(" ", ""), "0").ToString & "', PrixCorrigeOffre='" & IIf(TxtPrixTotal.Text.Replace(" ", "") <> "", TxtPrixTotal.Text.Replace(" ", ""), "0").ToString & "', ErreurCalcul='" & IIf(TxtErrCalcul.Text.Replace(" ", "") <> "", TxtErrCalcul.Text.Replace(" ", ""), "0").ToString & "',RangExamDetaille='0' where RefSoumis='" & ReponseDialog & "'"
                    ExecuteNonQuery(query)
                Else
