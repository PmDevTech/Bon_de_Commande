...
Public Class PlanMarcheSuite
    ...
    Private Sub PlanMarcheSuite_Load(ByVal sender As Object, ByVal e As EventArgs) Handles MyBase.Load
        ...
        'dtEtape.Columns.Add("N°", Type.GetType("System.String"))
    ...
    Private Sub LoadMarche(ByVal Bailleur As String, ByVal CodeConvention As String)
        ...
        If PlanMarche.ModePPM = "Genere" Then
            If PlanMarche.ElaboPPM = "Tous les bailleurs" Then
                query = "Select * from T_Marche where CodeProjet='" & ProjetEnCours & "' AND RefPPM='" & RefPPM & "' AND ModePPM ='Tous_Bailleurs' ORDER BY DescriptionMarche"
            Else
                query = "Select * from T_Marche where CodeProjet='" & ProjetEnCours & "' AND InitialeBailleur='" & Bailleur & "' AND CodeConvention='" & CodeConvention & "' AND RefPPM='" & RefPPM & "'  AND ModePPM ='Bailleur' ORDER BY DescriptionMarche"
            End If
        Else
            query = "Select * from T_Marche where CodeProjet='" & ProjetEnCours & "' AND RefPPM='" & RefPPM & "' AND ModePPM ='PPSD' ORDER BY RefMarche"
        End If
        Dim dt As DataTable = ExcecuteSelectQuery(query)

        For Each rw As DataRow In dt.Rows
            AddMarcheItem(rw("RefMarche"), rw("TypeMarche"))
        Next
    End Sub
    ...
    Private Sub AddMarcheItem(ByVal RefMarche As String, ByVal TypeMarche As String)
        ...
        If TypeMarche = "Consultants" Then
        ...
        ElseIf TypeMarche = "Fournitures" Then
            Dim NewMarche As New DevExpress.XtraNavBar.NavBarItem With {
               .Caption = LibelleMarche, .Name = RefMarche
           }
            Dim NewLink = NavBarFournitures.ItemLinks.Add(NewMarche)
            NavBarFournitures.ItemLinks.Add(NewLink)
        ElseIf TypeMarche = "Services autres que les services de consultants" Then
            Dim NewMarche As New DevExpress.XtraNavBar.NavBarItem With {
               .Caption = LibelleMarche, .Name = RefMarche
           }
    ...
    Private Sub GetInfoMarche(ByVal RefMarche As String)
        ...
        For Each rw As DataRow In dt.Rows
            ...
            If rw("TypeMarche").ToString = "Consultants" Then
                If Val(ExecuteScallar("SELECT COUNT(*) FROM t_marche as M, t_ami as D WHERE M.RefMarche=D.RefMarche AND M.RefMarche='" & RefMarche & "'")) > 0 Then StatuMarche = True
                If Val(ExecuteScallar("SELECT COUNT(*) FROM t_marche as M, t_dp as D WHERE M.RefMarche=D.RefMarche AND D.Statut<>'Annuler' AND M.RefMarche='" & RefMarche & "'")) > 0 Then StatuMarche = True
            Else
                If Val(ExecuteScallar("SELECT COUNT(*) FROM t_marche as M, t_dao as D WHERE D.statut_DAO<>'Annuler' AND M.RefMarche=D.RefMarche AND M.RefMarche='" & RefMarche & "'")) > 0 Then StatuMarche = True
            End If
    ...
    Private Sub btSave_Click(sender As Object, e As EventArgs) Handles btSave.Click
        Try
            ...
            If Not VerifEtapePlan(CurrentRefMarche) Then
                ...
                'If ViewEtape.GetRowCellValue(0, "Début").ToString = "" Then
                '    SuccesMsg("Veuillez renseigner la date de début à la ligne 1")
                '    Exit Sub
                'End If
                DebutChargement(True, "Enregistrement en cours...")

                query = "UPDATE t_marche SET DescriptionMarche='" & EnleverApost(txtMarche.Text.Trim()) & "', RevuePrioPost='" & cmbRevue.Text & "' WHERE CodeProjet='" & ProjetEnCours & "' AND RefMarche='" & CurrentRefMarche & "'"
