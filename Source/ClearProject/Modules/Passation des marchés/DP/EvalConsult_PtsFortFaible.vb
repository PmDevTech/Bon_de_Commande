...
Public Class EvalConsult_PtsFortFaible
    ...
    Private Sub EvalConsult_PtsFortFaible_Load(ByVal sender As System.Object, ByVal e As System.EventArgs)  Handles MyBase.Load
    ...
    Private Sub ChargerConsult()
        ...
        query = "select C.NomConsult from T_Consultant as C,T_SoumissionConsultant as S where S.RefConsult=C.RefConsult and C.NumeroDp='" & EvaluationConsultants.CmbNumDoss.Text & "' order by C.NomConsult"
    ...
    Private Sub CmbConsult_SelectedValueChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles CmbConsult.SelectedValueChanged
        ...
        Dim Code As String = ""
        query = "select RefConsult from T_Consultant where NumeroDp='" & EvaluationConsultants.CmbNumDoss.Text & "' and NomConsult='" & EnleverApost(CmbConsult.Text) & "'"
        Dim dt As DataTable = ExcecuteSelectQuery(query)
        For Each rw As DataRow In dt.Rows
            Code = rw(0).ToString
        Next

        query = "select S.Refsoumis from T_Consultant as C,T_SoumissionConsultant as S where C.RefConsult=S.RefConsult and C.RefConsult='" & Code & "' and C.NumeroDp='" & EvaluationConsultants.CmbNumDoss.Text & "'"
    ...
    Private Sub ChargerNoteConsult()
        ...
        query = "select RefCritere,CodeCritere,IntituleCritere,TypeCritere,PointCritere from T_DP_CritereEval where NumeroDp='" & EvaluationConsultants.CmbNumDoss.Text & "' and CritereParent='0' order by RefCritere"
        Dim dt2 As DataTable = ExcecuteSelectQuery(query)
        For Each rw2 As DataRow In dt2.Rows
            Dim drS = dtNoteCons.NewRow()

            drS(0) = "x"
            drS(1) = MettreApost(rw2(1).ToString & "/ " & rw2(2).ToString)
            drS(2) = IIf(rw2(3).ToString = "Note", Note(rw2(0).ToString).ToString & " / " & rw2(4).ToString, "").ToString

            dtNoteCons.Rows.Add(drS)

            If (rw2(3).ToString <> "Note") Then

                query = "select RefCritere,CodeCritere,IntituleCritere,TypeCritere,PointCritere from T_DP_CritereEval where NumeroDp='" & EvaluationConsultants.CmbNumDoss.Text & "' and CritereParent='" & rw2(0).ToString & "' order by RefCritere"
                Dim dt As DataTable = ExcecuteSelectQuery(query)
                For Each rw As DataRow In dt.Rows
                    ...
                    drS1(0) = ""
                    drS1(1) = MettreApost(rw(1).ToString & "/ " & rw(2).ToString)
                    drS1(2) = IIf(rw(3).ToString = "Note", Note(rw(0).ToString).ToString & " / " & rw(4).ToString, "").ToString
                    ...
                    If (rw(3).ToString <> "Note") Then
                        ...
                        query = "select RefCritere,CodeCritere,IntituleCritere,TypeCritere,PointCritere from T_DP_CritereEval where NumeroDp='" & EvaluationConsultants.CmbNumDoss.Text & "' and CritereParent='" & rw(0).ToString & "' order by RefCritere"
                        Dim dt1 As DataTable = ExcecuteSelectQuery(query)
                        For Each rw1 As DataRow In dt1.Rows

                            Dim drS10 = dtNoteCons.NewRow()
                            drS10(0) = ""
                            drS10(1) = MettreApost(rw1(1).ToString & "/ " & rw1(2).ToString)
                            drS10(2) = IIf(rw1(3).ToString = "Note", Note(rw1(0).ToString).ToString & " / " & rw1(4).ToString, "").ToString
    ...
    Private Function Note(ByVal Critere As String) As Decimal
        ...
        query = "select Count(*) from T_Commission where NumeroDAO='" & EvaluationConsultants.CmbNumDoss.Text & "' and TypeComm='EVAC'"
