...
Public Class DiagChoixTypeFourniture
    ...
    Private Sub ChargerGrid(Optional TextRecherche As String = "")
        ...
        For Each rw As DataRow In dt.Rows
            Dim drS1 = dts.NewRow()
            drS1("IdItem") = rw("IdCat").ToString
            drS1("Libellé") = MettreApost(rw("LibelleCat"))
            drS1("Catégorie") = MettreApost(rw("LibelleCat"))
            drS1("Sous catégorie") = ""
            drS1("Type") = "Cat"
            drS1("Edit") = False
            dts.Rows.Add(drS1)
            ...
            For Each rwSousCat As DataRow In dtSousCat.Rows
                Dim drS2 = dts.NewRow()
                drS2("IdItem") = rwSousCat("IdSousCat").ToString
                drS2("Libellé") = "     - " & MettreApost(rwSousCat("LibelleSousCat"))
                drS2("Catégorie") = MettreApost(rw("LibelleCat"))
                drS2("Sous catégorie") = MettreApost(rwSousCat("LibelleSousCat"))
                drS2("Type") = "SousCat"
                drS2("Edit") = False
                dts.Rows.Add(drS2)
            Next
    ...
    Private Sub cmbCategorie_KeyDown(sender As Object, e As KeyEventArgs) Handles cmbCategorie.KeyDown
        If e.KeyCode = Keys.Enter Then
            ...
            Else
                ...
                LoadComboCategorie()
                Dim LastID As Decimal = Val(ExecuteScallar(query))
                Dim dts As DataTable = GridCategorie.DataSource
                Dim drS1 = dts.NewRow()
                drS1(0) = LastID
